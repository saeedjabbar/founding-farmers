...
- [x] Centralize Blocks rendering via `StrapiRichText` so all rich text fields consume the JSON Blocks schema. Record detail pages now pull an optional `searchableContent` Blocks field to render the right-hand transcript panel, with an inline search box that highlights matches and allows navigation across matched terms.
